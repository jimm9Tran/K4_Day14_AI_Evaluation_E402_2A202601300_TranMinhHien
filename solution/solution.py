...
class LLMJudge:
    ...
    def _build_prompt(self, question: str, answer: str, rubric: dict[str, Any]) -> str:
        ...
        return (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{criteria}\n\n"
            "Score each criterion from 0.0 to 1.0 and reply with JSON only, "
            'e.g. {"criterion_name": 0.8}.'
        )
    ...
    def score_response(
        ...
        try:
            ...
            if not isinstance(parsed, dict) or not all(
                isinstance(value, (int, float)) for value in parsed.values()
            ):
                raise ValueError("Judge response is not a flat JSON score map")
            scores = {criterion: float(value) for criterion, value in parsed.items()}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Assumes scores_batch alternates "first-shown" / "second-shown"
        # responses from a position-swap experiment (Exercise 1.2, Câu 1):
        # even indices are the response judged first, odd indices second.
        first_position = entry_averages[0::2]
        second_position = entry_averages[1::2]
        positional_bias = False
        if first_position and second_position:
            positional_bias = (
                sum(first_position) / len(first_position)
                - sum(second_position) / len(second_position)
            ) > 0.2
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avgs = {metric: _avg(new_results, metric) for metric in metrics}
        baseline_avgs = {metric: _avg(baseline_results, metric) for metric in metrics}
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        by_category = {
            "hallucination": (
                "Implement a hallucination checker that filters claims unsupported "
                "by retrieved context"
            ),
            "irrelevant": (
                "Improve prompt and routing so the assistant addresses the actual "
                "question asked"
            ),
            "incomplete": (
                "Increase chunk size or retrieved context window to reduce missing "
                "information"
            ),
            "off_topic": "Improve intent detection so the assistant stays on the requested topic",
            "refusal": "Relax overly strict guardrails that cause unnecessary refusals",
        }
        suggestions = [
            text for failure_type, text in by_category.items() if categories.get(failure_type)
        ]

        fallback_suggestions = [
            "Add few-shot examples showing complete, well-grounded answers to guide generation",
            "Expand the golden dataset with more edge cases similar to the observed failures",
            "Review and tighten the system prompt to reduce ambiguity in edge cases",
        ]
